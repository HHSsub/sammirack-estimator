...
export const generateInventoryPartId = (item) => {
  ...
  if (specification && String(specification).trim()) {
    ...
    // ✅ 스텐랙 선반 전용: specification에서 W(너비)만 추출
    // 재고관리 측면에서 D(깊이)는 의미가 없으므로 W만 사용
    // 예: "사이즈43x90" → "43", "사이즈50x75" → "50"
    if (rackType === '스텐랙' && cleanName === '선반') {
      const sizeMatch = cleanSpec.match(/사이즈\s*(\d+)x\d+/i) || 
                        cleanSpec.match(/(\d+)x\d+/);
      if (sizeMatch) {
        cleanSpec = sizeMatch[1]; // W만 추출
      }
    }
...
export const EXTRA_TO_BASE_INVENTORY_MAPPING = {
  ...
  // ✅ 스텐랙 선반: 재고관리는 W(너비)만 구분 (50x75, 50x90 등 모두 선반-50으로 통합)
  '스텐랙-50x75선반-': '스텐랙-선반-50',
  '스텐랙-50x90선반-': '스텐랙-선반-50',
  '스텐랙-50x120선반-': '스텐랙-선반-50',
  '스텐랙-50x150선반-': '스텐랙-선반-50',
  '스텐랙-50x180선반-': '스텐랙-선반-50',
  // ✅ 스텐랙 선반 43 사이즈 (있다면)
  '스텐랙-43x90선반-': '스텐랙-선반-43',
  '스텐랙-43x120선반-': '스텐랙-선반-43',
  '스텐랙-43x150선반-': '스텐랙-선반-43',
  '스텐랙-43x180선반-': '스텐랙-선반-43',
