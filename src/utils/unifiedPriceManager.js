...
export const generatePartId = (item) => {
  ...
  // 하이랙 특수 처리: 품명 우선 순서 보장
  if (rackType === '하이랙') {
    let baseName = '';
    if (cleanName.includes('기둥')) baseName = '기둥';
    else if (cleanName.includes('선반')) baseName = '선반';
    else if (cleanName.includes('로드빔') || cleanName.includes('빔')) baseName = '로드빔';
...
export const generateInventoryPartId = (item) => {
  ...
  // ✅ 하이랙 전용: DB Key 구조 완벽 재현 ([품명][색상][속성][중량])
  if (rackType === '하이랙') {
    let baseName = '';
    if (cleanName.includes('기둥')) baseName = '기둥';
    else if (cleanName.includes('선반')) baseName = '선반';
    else if (cleanName.includes('로드빔')) baseName = '로드빔';
    else baseName = cleanName;
    ...
    // 중량 추출 (기본 270kg)
    let weightAttr = '270kg';
    if (cleanName.includes('450kg') || String(specification).includes('450kg')) weightAttr = '450kg';
    else if (cleanName.includes('600kg') || String(specification).includes('600kg')) weightAttr = '600kg';

    // 규격 처리 및 중복 중량 제거
    let cleanSpec = String(specification || '')
      .replace(/\s+/g, '')
      .replace(/\*/g, 'x')
      .replace(/(270|450|600)kg/g, ''); // 파싱 전 중량 제거

    let finalSpec = cleanSpec;
    ...
    if (baseName === '기둥') {
      ...
      const sizePart = sizeMatch ? sizeMatch[1] : '45x108';
      finalSpec = `사이즈${sizePart}${weightAttr}`;
    } else if (baseName === '로드빔') {
      // 로드빔: [길이][중량] (DB: 108270kg)
      const lengthMatch = cleanSpec.match(/(\d+)/);
      const lengthPart = lengthMatch ? lengthMatch[1] : '108';
      finalSpec = `${lengthPart}${weightAttr}`;
    }

    // 최종 결과: 하이랙-[품명][색상+속성][중량]-[규격]
    return `하이랙-${baseName}${colorAttr}${weightAttr}-${finalSpec}`;
  }
...
export const EXTRA_TO_BASE_INVENTORY_MAPPING = {
  ...
  // ========================================
  // 하이랙 270kg 메트그레이 선반 매핑 (6개)
  // ========================================
  '하이랙-선반45x108매트그레이-': '하이랙-선반메트그레이(볼트식)270kg-사이즈45x108270kg',
  '하이랙-선반45x150매트그레이-': '하이랙-선반메트그레이(볼트식)270kg-사이즈45x150270kg',
  '하이랙-선반45x200매트그레이-': '하이랙-선반메트그레이(볼트식)270kg-사이즈45x200270kg',
  '하이랙-선반60x108매트그레이-': '하이랙-선반메트그레이(볼트식)270kg-사이즈60x108270kg',
  '하이랙-선반60x150매트그레이-': '하이랙-선반메트그레이(볼트식)270kg-사이즈60x150270kg',
  '하이랙-선반60x200매트그레이-': '하이랙-선반메트그레이(볼트식)270kg-사이즈60x200270kg',

  // ========================================
  // 하이랙 270kg 오렌지 선반 매핑 (6개)
  // ========================================
  '하이랙-선반45x108-': '하이랙-선반블루(기둥)+오렌지(가로대)(볼트식)270kg-사이즈45x108270kg',
  '하이랙-선반45x150-': '하이랙-선반블루(기둥)+오렌지(가로대)(볼트식)270kg-사이즈45x150270kg',
  '하이랙-선반45x200-': '하이랙-선반블루(기둥)+오렌지(가로대)(볼트식)270kg-사이즈45x200270kg',
  '하이랙-선반60x108-': '하이랙-선반블루(기둥)+오렌지(가로대)(볼트식)270kg-사이즈60x108270kg',
  '하이랙-선반60x150-': '하이랙-선반블루(기둥)+오렌지(가로대)(볼트식)270kg-사이즈60x150270kg',
  '하이랙-선반60x200-': '하이랙-선반블루(기둥)+오렌지(가로대)(볼트식)270kg-사이즈60x200270kg',

  // ========================================
  // 하이랙 270kg 블루 기둥 매핑 (4개)
  // ========================================
  '하이랙-기둥45x150-': '하이랙-기둥블루(기둥)+오렌지(가로대)(볼트식)270kg-사이즈45x높이150270kg',
  '하이랙-기둥45x200-': '하이랙-기둥블루(기둥)+오렌지(가로대)(볼트식)270kg-사이즈45x높이200270kg',
  '하이랙-기둥60x150-': '하이랙-기둥블루(기둥)+오렌지(가로대)(볼트식)270kg-사이즈60x높이150270kg',
  '하이랙-기둥60x200-': '하이랙-기둥블루(기둥)+오렌지(가로대)(볼트식)270kg-사이즈60x높이200270kg',

  // ========================================
  // 하이랙 270kg 메트그레이 기둥 매핑 (4개) - 추가상품3에 포함
  // ========================================
  '하이랙-기둥45x150매트그레이-': '하이랙-기둥메트그레이(볼트식)270kg-사이즈45x높이150270kg',
  '하이랙-기둥45x200매트그레이-': '하이랙-기둥메트그레이(볼트식)270kg-사이즈45x높이200270kg',
  '하이랙-기둥60x150매트그레이-': '하이랙-기둥메트그레이(볼트식)270kg-사이즈60x높이150270kg',
  '하이랙-기둥60x200매트그레이-': '하이랙-기둥메트그레이(볼트식)270kg-사이즈60x높이200270kg',

  // ========================================
  // 하이랙 270kg 아이보리 기둥 매핑 (60x만, 45x 없음)
  // ========================================
  '하이랙-기둥60x150아이보리-': '하이랙-기둥아이보리(볼트식)270kg-사이즈60x높이150270kg',
  '하이랙-기둥60x200아이보리-': '하이랙-기둥아이보리(볼트식)270kg-사이즈60x높이200270kg',
  '하이랙-기둥60x250아이보리-': '하이랙-기둥아이보리(볼트식)270kg-사이즈60x높이250270kg',

  // ========================================
  // 하이랙 270kg 아이보리 선반 매핑 (60x만)
  // ========================================
  '하이랙-선반60x108아이보리-': '하이랙-선반아이보리(볼트식)270kg-사이즈60x108270kg',
  '하이랙-선반60x150아이보리-': '하이랙-선반아이보리(볼트식)270kg-사이즈60x150270kg',
  '하이랙-선반60x200아이보리-': '하이랙-선반아이보리(볼트식)270kg-사이즈60x200270kg',

  // ========================================
  // 하이랙 450kg 매핑 - 추가상품4 (메트그레이)
  // ========================================
  '하이랙-기둥60x150450kg-': '하이랙-기둥메트그레이(볼트식)450kg-사이즈60x높이150450kg',
  '하이랙-기둥60x200450kg-': '하이랙-기둥메트그레이(볼트식)450kg-사이즈60x높이200450kg',
  '하이랙-기둥60x250450kg-': '하이랙-기둥메트그레이(볼트식)450kg-사이즈60x높이250450kg',
  '하이랙-선반60x108450kg-': '하이랙-선반메트그레이(볼트식)450kg-사이즈60x108450kg',
  '하이랙-선반60x150450kg-': '하이랙-선반메트그레이(볼트식)450kg-사이즈60x150450kg',
  '하이랙-선반60x200450kg-': '하이랙-선반메트그레이(볼트식)450kg-사이즈60x200450kg',
...
export const EXTRA_TO_BASE_PARTID_MAPPING = {
  ...
  // 하이랙 270kg 블루 기둥
  // ========================================
  '하이랙-45x150기둥-': '하이랙-기둥-사이즈45x높이150270kg',
  '하이랙-45x200기둥-': '하이랙-기둥-사이즈45x높이200270kg',
  '하이랙-60x150기둥-': '하이랙-기둥-사이즈60x높이150270kg',
  '하이랙-60x200기둥-': '하이랙-기둥-사이즈60x높이200270kg',

  // ========================================
  // 하이랙 270kg 메트그레이 기둥
  // ========================================
  '하이랙-45x150메트그레이기둥-': '하이랙-기둥-사이즈45x높이150270kg',
  '하이랙-45x200메트그레이기둥-': '하이랙-기둥-사이즈45x높이200270kg',
  '하이랙-60x150메트그레이기둥-': '하이랙-기둥-사이즈60x높이150270kg',
  '하이랙-60x200메트그레이기둥-': '하이랙-기둥-사이즈60x높이200270kg',

  // ========================================
  // 하이랙 270kg 아이보리 기둥 (60x만, partId)
  // ========================================
  '하이랙-60x150아이보리기둥-': '하이랙-기둥-사이즈60x높이150270kg',
  '하이랙-60x200아이보리기둥-': '하이랙-기둥-사이즈60x높이200270kg',
  '하이랙-60x250아이보리기둥-': '하이랙-기둥-사이즈60x높이250270kg',

  // ========================================
  // 하이랙 270kg 아이보리 선반 (60x만, partId)
  // ========================================
  '하이랙-60x108아이보리선반-': '하이랙-선반-사이즈60x108270kg',
  '하이랙-60x150아이보리선반-': '하이랙-선반-사이즈60x150270kg',
  '하이랙-60x200아이보리선반-': '하이랙-선반-사이즈60x200270kg',

  // ========================================
  // 하이랙 450kg
  // ========================================
  '하이랙-60x150기둥450kg-': '하이랙-기둥-사이즈60x높이150450kg',
  '하이랙-60x200기둥450kg-': '하이랙-기둥-사이즈60x높이200450kg',
  '하이랙-60x250기둥450kg-': '하이랙-기둥-사이즈60x높이250450kg',
