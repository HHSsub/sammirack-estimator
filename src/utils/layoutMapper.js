...
export const createEstimateLayout = (data, type = 'estimate') => {
  ...
  // 고객 정보 섹션
  layout[5][0] = '거래일자';
  layout[5][2] = data.date || new Date().toISOString().slice(0, 10);
  
  layout[6][0] = '상호명';
  layout[6][2] = data.customerName || '';
  
  layout[7][0] = '담당자';
  layout[7][2] = data.contactPerson || '';
  ...
  // 명세 데이터 입력 (OptionSelector에서 온 실제 제품 데이터 사용)
  if (data.selectedProducts && data.selectedProducts.length > 0) {
    data.selectedProducts.forEach((product, index) => {
      if (index < 13) { // 최대 13개 항목 (A13~A25)
        ...
        layout[rowIndex][1] = product.name || product.title || ''; // 품명
        layout[rowIndex][2] = 'EA'; // 단위
        layout[rowIndex][3] = product.quantity || 1; // 수량
        layout[rowIndex][4] = product.price || 0; // 단가
        layout[rowIndex][5] = (product.quantity || 1) * (product.price || 0); // 공급가
        layout[rowIndex][6] = product.description || ''; // 비고
      }
    });
  } else {
    // 기본 번호만 입력 (1~13)
  ...
  // 실제 계산된 값들
  const subtotal = calculateSubtotal(data.selectedProducts || []);
...
export const validateAndCleanData = (rawData, type) => {
  const cleanData = {
    ...
    contactPerson: rawData.contactPerson || rawData.contact || '',
    specialNotes: rawData.specialNotes || rawData.notes || '',
    selectedProducts: [], // OptionSelector에서 온 실제 제품 데이터
    materials: [] // 발주서용 원자재 데이터
  };
  
  // OptionSelector에서 온 실제 제품 데이터 정제
  if (rawData.selectedProducts && Array.isArray(rawData.selectedProducts)) {
    cleanData.selectedProducts = rawData.selectedProducts.map(product => ({
    ...
    cleanData.selectedProducts = rawData.items.map(item => ({
      name: item.name || item.productName || '',
      quantity: parseFloat(item.quantity) || 1,
      price: parseFloat(item.price) || parseFloat(item.unitPrice) || 0,
      description: item.note || item.remarks || ''
    }));
  }
  
  // materials 데이터 정제 (발주서용)
  if (rawData.materials && Array.isArray(rawData.materials)) {
    cleanData.materials = rawData.materials.map(material => ({
      name: material.name || material.partName || '',
      quantity: parseFloat(material.quantity) || 0,
      price: parseFloat(material.price) || parseFloat(material.unitPrice) || 0,
      note: material.note || material.remarks || ''
    }));
  }
...
// 파일명 생성 함수 (빌드 에러 해결)
export const generateFileName = (type, customerName, date) => {
  const typeNames = {
    'estimate': '견적서',
    'transaction': '거래명세서',
    'purchase': '발주서'
  ...
  const documentType = typeNames[type] || '문서';
  const formattedDate = date || new Date().toISOString().slice(0, 10);
  const customer = customerName || '고객';
  
  return `${documentType}_${customer}_${formattedDate}.xlsx`;
};
