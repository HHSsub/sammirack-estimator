...
const PurchaseOrderForm = () => {
  ...
  const proceedWithSave = async (itemId, existingDoc) => {
    ...
    if (success) {
      ...
      setToast({
        show: true,
        message: isEditMode ? '발주서가 수정되었습니다.' : '발주서가 저장되었습니다.',
        type: 'success'
      });
  ...
  const proceedWithPrint = async () => {
    const title = `발주서_${formData.documentNumber}`;
    ...
    setTimeout(async () => {
      ...
      if (confirmDeduct && cart && cart.length > 0) {
        ...
        if (hasBom || (formData.materials && formData.materials.length > 0)) {
          ...
          const result = await deductInventoryOnPrint(syntheticCart, '발주서', formData.documentNumber, undefined);
        ...
        // ✅ cart에 bom 없으면 formData.materials를 cart 형식으로 변환
          ...
          if (materialsAsCart.length > 0) {
            const result = await deductInventoryOnPrint(materialsAsCart, '발주서', formData.documentNumber, undefined);
      ...
      if (result.success) {
        ...
        if (cart && cart.length > 0) {
          const materialsForDeductFax = (cart.every(i => !i.bom?.length) && formData.materials?.length)
            ...
            : undefined;
          const deductResult = await deductInventoryOnPrint(cart, '발주서(FAX)', formData.documentNumber, materialsForDeductFax);
