...
const PurchaseOrderForm = () => {
  ...
  const handlePrint = () => {
    ...
    localStorage.setItem(`order_${id}`, JSON.stringify(historyItem));
    // ========================
      ...
      <div className="mb-6 p-4 border rounded">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div><label>발주번호</label>
            <input type="text" name="orderNumber" value={formData.orderNumber} onChange={handleInputChange} placeholder="자동 생성" className="w-full p-2 border rounded"/>
          </div>
          <div><label>상호명 (공급받는 쪽)</label>
            <input type="text" name="companyName" value={formData.companyName} onChange={handleInputChange} className="w-full p-2 border rounded"/>
          </div>
          <div><label>담당자</label>
            <input type="text" name="contactPerson" value={formData.contactPerson} onChange={handleInputChange} className="w-full p-2 border rounded"/>
          </div>
          <div><label>고객명</label>
      ...
      <div className="mb-4">
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody>
            {cart.map((item, idx) => (
              <tr key={idx}>
                <td>{item.displayName || `${item.type} ${item.options?.size || ''}`} × {item.quantity}개</td>
