...
export const ProductProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // 🔸 각 타입 독립 처리 (파렛트랙 ↔ 파렛트랙 철판형 교차 병합 제거)
    ...
    if (selectedType === '하이랙' && data?.하이랙) {
      ...
      if (selectedOptions.color) {
        ...
        const sizeViewList = sizeListSafeRaw.map(s =>
          (is550 && HIGHRACK_550_ALIASES_VIEW_FROM_DATA[s]) ? HIGHRACK_550_ALIASES_VIEW_FROM_DATA[s] : s
        );

        let sizeView = is450 ? sizeViewList.filter(s => s !== '45x150') : sizeViewList;

        // 비-최대하중(=450kg 등)일 때는 보조 사이즈 병합
        ...
        // 550kg이면 80x200 강제 추가(없는 데이터 보조)
        ...
        if (selectedOptions.size) {
          ...
          // 450kg에서도 150 반드시 노출
          let mergedHeights = Array.from(new Set([...heightListSafe, ...EXTRA_OPTIONS['하이랙'].height]));
          // 550kg + 80x200일 때도 150/200/250 모두 노출 보조
          if (is550 && selectedOptions.size === '80x200') {
            mergedHeights = Array.from(new Set([...mergedHeights, '150','200','250']));
          }
          opts.height = sortHeights(mergedHeights);
          ...
          if (selectedOptions.height) {
            ...
            if (levelsFromData.length > 0) {
              opts.level = sortLevels(levelsFromData);
            } else {
              // 데이터가 없으면 보조 레벨로 폴백
              opts.level = sortLevels([...(EXTRA_OPTIONS['하이랙'].level || []), ...COMMON_LEVELS]);
            }
          }
        }
      }
  ...
  // ▶ 컴포넌트 합산으로 1세트 가격을 산출
  const sumFromComponents = (rec) => {
    if (!rec || !Array.isArray(rec.components)) return 0;
    let sum = 0;
    for (const c of rec.components) {
      const unit = Number(c.unit_price) || 0;
      const t    = Number(c.total_price) || 0;
      const q    = Number(c.quantity) || 0;
      sum += t > 0 ? t : (unit > 0 ? unit * q : 0);
    }
    return sum;
  };
  ...
  const calculatePrice = useCallback(() => {
    ...
    if (formTypeRacks.includes(selectedType)) {
      ...
      let rec;
      if (selectedType === '경량랙' && selectedOptions.height === 'H750') {
        rec = bomData['경량랙']?.[selectedOptions.size]?.['H900']
          ?. [selectedOptions.level]?.[selectedOptions.formType];
      } else {
        rec = bomData[selectedType]?.[selectedOptions.size]?.[selectedOptions.height]
          ?. [selectedOptions.level]?.[selectedOptions.formType];
      }

      // 1) total_price 우선
      if (rec && Number(rec.total_price) > 0) {
        basePrice = Number(rec.total_price);
      } else {
        // 2) 없거나 0이면 컴포넌트 합산
        basePrice = sumFromComponents(rec);
      }
      basePrice *= (Number(quantity) || 0);
    } else if (selectedType === '스텐랙') {
      const p = data['스텐랙']['기본가격']?.[selectedOptions.size]?.[selectedOptions.height]?.[selectedOptions.level];
      if (p) basePrice = p * (Number(quantity) || 0);
    ...
    // ▸ 일반 extra 옵션(체크박스) + 경량 사용자 정의 자재 가격 (옵션 가격은 세트당 가격으로 가정)
    let extraPricePerSet = 0;
    const exBlock = getExtraForType(selectedType, extraProducts);
    if (exBlock && extraOptionsSel?.length) {
      Object.values(exBlock).forEach(catArr => {
        catArr.forEach(opt => {
          if (extraOptionsSel.includes(opt.id)) {
            extraPricePerSet += Number(opt.price) || 0;
          }
        });
      });
    }
    extraPricePerSet += (customMaterials || []).reduce((s, m) => s + (Number(m.price) || 0), 0);

    const total = basePrice + extraPricePerSet * (Number(quantity) || 0);
    return Math.round(total * (applyRate / 100));
  }, [
    ...
    if (ex && extraOptionsSel?.length) {
      Object.values(ex).forEach(catArr => {
        catArr.forEach(opt => {
          ...
          // 🧩 보조 규칙: 옵션 이름에 "타이빔2포함/타이빔4포함" 등이 있으면 타이빔 자동 추가
          const m = String(opt.name).match(/타이빔\s*([0-9]+)\s*포함|타이빔([0-9]+)/);
          const tieCount = m ? Number(m[1] || m[2] || 0) : 0;
          if (tieCount > 0 && qty > 0) {
            result.push({
              rackType: selectedType,
              name: '타이빔',
              specification: '',
              quantity: tieCount * qty,
              unitPrice: 0,
              totalPrice: 0,
              note: '추가옵션(포함)'
            });
    ...
    if (selectedType === '파렛트랙' || selectedType === '파렛트랙 철판형') {
      ...
      const base = [
        ...
        ...(form === '연결형'
          ...
          : [
              { rackType:selectedType, size:sz, name:'베이스(안전좌)', specification:'', quantity:baseSafetyLeftQty, unitPrice:2500, totalPrice:0 },
              { rackType:selectedType, size:sz, name:'베이스(안전우)', specification:'', quantity:baseSafetyRightQty, unitPrice:2500, totalPrice:0 }
            ]),
  ...
  const addToCart = () => {
    ...
    setCart(prev => [...prev, {
      ...
      extraOptions: [...extraOptionsSel], // 선택한 추가옵션 유지
      quantity,
